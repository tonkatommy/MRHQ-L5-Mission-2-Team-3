...
describe("POST /api/v1/getQuote", () => {
  it("returns correct quote", async () => {
    const body = { car_value: 6614, risk_rating: 5 };
    const res = await request(app).post("/api/v1/getQuote").send(body);
  ...
  it("handles risk ratings not between 1 and 5", async () => {
    ...
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty("error", "risk rating must be between 1 and 5");
  });
